...
class PlayState extends MusicBeatState
{
	...
	public static var ratingStuff:Array<Dynamic> = [
		['Are u adopted', 0.2], //From 0% to 19%
		['Quit', 0.4], //From 20% to 39%
		['Thats enough for u', 0.5], //From 40% to 49%
		['Bro how', 0.6], //From 50% to 59%
		['How you this bad', 0.69], //From 60% to 68%
		['Kinda shit', 0.7], //69%
		['Average', 0.8], //From 70% to 79%
		['wow so pro', 0.9], //From 80% to 89%
		['ratio', 1], //From 90% to 99%
		['Fallnnn rank!!', 1] //The value on this one isn't used actually, since Perfect is always "1"
	]; 
	...
	override public function create()
	{
		...
		{
			case 'where-are-you' | 'eruption' | 'kaio-ken' | 'whats-new' | 'blast' | 'surge'| 'spark'| 'double-god-gateway'| 'super-saiyan' | 'overflow':
				defaultCamZoom = 0.9;
				curStage = 'mansion';
			...
			case 'astral-calamity' | 'talladega'| 'big-shot':
				...
				add(ground);
					case 'purple-power':
				defaultCamZoom = 0.56;
				if (SONG.song == 'purple-power') defaultCamZoom = 0.6;

				curStage = 'purple';

				var bgbg:BGElement = new BGElement('WBG/crystal', -590, -500,  1, 1, 4);
				add(bgbg);

				var lavaLimits:BGElement = new BGElement('WBG/base', -590, -500,  1, 1, 4);
				add(lavaLimits);

				var bgSpikes:BGElement = new BGElement('WBG/circle', -590, -500,  1, 1, 4);
				add(bgSpikes);
					
			case 'mild-mania':
				defaultCamZoom = 0.56;
				if (SONG.song == 'mild-mania') defaultCamZoom = 0.7;
		...
		if (isStoryMode)
		{
			...
			{	
				...
				case 'cutscene':
					...
					video.playMP4(Paths.video('purple'));
